...
async function loadClaims() {
    ...
    try {
        ...
        const response = await fetch('view_claims.php?' + params.toString());
        const data = await response.json();
...
function renderClaimsTable(claims) {
    ...
    for (let i = 0; i < claims.length; i++) {
        ...
        const lostReportBadge = claim.has_lost_report 
            ? '<span class="badge-lost" title="Linked to lost report"><i class="fas fa-link"></i> Linked</span>' 
            : '';
...
async function viewClaimDetails(claimId) {
    try {
        ...
        if (data.success) {
            ...
            if (reviewBtn) {
                reviewBtn.href = 'review_claims.php?id=' + claimId;
            }
        } else {
...
function showClaimModal(claim) {
    ...
    if (claim.lost_report_id && claim.lost_item_name) {
        lostReportHtml = `
            <div class="detail-row">
                <div class="detail-label">Linked Lost Report</div>
                <div class="detail-value">
                    <strong>${escapeHtml(claim.lost_item_name)}</strong><br>
                    <small>Lost at: ${escapeHtml(claim.lost_location)} on ${escapeHtml(claim.lost_date)}</small>
                </div>
            </div>
        `;
    }
    ...
    modalBody.innerHTML = `
        ...
        <div class="detail-row">
            ...
            <div class="detail-value">${claim.ownership_proof || 'Not provided'}</div>
        </div>
        <div class="detail-row">
            <div class="detail-label">Identifying Details</div>
            <div class="detail-value">${claim.identifying_details || 'Not provided'}</div>
        </div>
        ${claim.review_note ? `
        <div class="detail-row">
